...
export class ConfigService {
    ...
    async addConfig(createConfigDto: CreateConfigDto) {
        try {
            const { _id, name, flag, allowedUsers, disallowedUsers, description, createdBy, editedBy } = createConfigDto as any;
            ...
            this.logger.log('The configuration has been registered successfully', { name, flag, allowedUsers, disallowedUsers });
    ...
    async getAllFlagsConfigs(page: number, rows: number) {
        try {
            const configs = await this.configModel.find({ flag: true })
                .sort({ createdAt: -1 })
                .skip(rows * (page - 1))
                .limit(rows);
    ...
    async updateConfig(id: string, updateConfigDto: UpdateConfigDto) {
        try {
            const { flag, name, allowedUsers, disallowedUsers, description, editedBy } = updateConfigDto;
    ...
    async validateUserInConfig(configId: string, userId: string) {
        try {
            ...
            throw new BadRequestException('The user is not in the list of allowed users or in the disallowed list of the config');
        } catch (error) {
            this.logger.error('Error when consulting the configs', { error });
            if (error instanceof NotFoundException || error instanceof BadRequestException) {
                throw error;
            }
            throw new InternalServerErrorException('Error when consulting the configs');
        }
    }
