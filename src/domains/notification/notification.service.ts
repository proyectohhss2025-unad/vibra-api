...
import { NOTIFICATION_CHANNEL_INBOX, NOTIFICATION_TYPE_ALERT, NOTIFICATION_TYPE_INFO } from 'src/utils/constants';
...
export class NotificationService {
    constructor(
        ...
        @InjectModel(NotificationType.name) private notificationTypeModel: Model<NotificationType>,
        @InjectModel(NotificationChannel.name) private notificationChannelModel: Model<NotificationChannel>,
     ...
     */
    async createGeneralNotification(message: string, title: string, user?: any): Promise<void> {
    ...
    async bulkCreate(file: Express.Multer.File) {
        ...
        for await (const row of parser) {
            ...
            const notificationType = await this.notificationTypeModel.findOne({ title: row['notificationType'] });
    ...
    async getByTitle(title: string) {
        return this.notificationModel.findOne({ title })
            .populate('user')
            .populate('notificationType')
            .populate('notificationChannel')
            .exec();
    ...
    async getAllByTitle(title: string, isRead: boolean, query: any) {
        return this.notificationModel.find({ title, isRead })
            .populate('notificationType')
            .skip(query.rows * (parseInt(query.page) - 1))
            .limit(parseInt(query.rows))
            .sort({ createdAt: -1 })
    ...
    async search(searchTerm: string) {
        ...
        }).populate('notificationType').exec();
    }
