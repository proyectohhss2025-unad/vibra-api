...
export class CompanyService {
  ...
  async create(createCompanyDto: CreateCompanyDto) {
    try {
      const { name, seriesCurrentBillingRange, ...companyData } =
        createCompanyDto;

      const existingCompany = await this.companyModel.findOne({ name });

      if (existingCompany) {
        this.logger.warn(`A company with name ${name} already exists`, {
          existingCompany,
        });
  ...
  async findByName(name: string) {
    try {
      ...
      this.logger.error(
        `Error querying company with name ${name}`,
        error.stack,
      );
      throw new InternalServerErrorException('Error when querying the company');
    }
  ...
  async findAll(page: number, rows: number) {
    try {
      ...
      throw new InternalServerErrorException(
        'Error when querying the companies',
      );
    }
  }
  ...
  async update(updateCompanyDto: UpdateCompanyDto) {
    try {
      ...
      return {
        message: 'The company update has been completed successfully',
        company,
      };
    } catch (error) {
      if (error instanceof NotFoundException) {
  ...
  async search(searchTerm: string, page: number, rows: number) {
    try {
      ...
      const objects = await this.companyModel
        .find(searchString === 'all' ? {} : query)
        .skip(rows * (page - 1))
        .limit(rows)
        .sort({ name: -1 });

      const count = await this.companyModel.countDocuments(
        searchString === 'all' ? {} : query,
      );
  ...
  async setActive(id: string, active: boolean, editedBy: string) {
    try {
      ...
      this.logger.log(
        `Company with ID ${id} active status updated to ${active}`,
      );
      ...
      this.logger.error(
        `Error updating active status for company with ID ${id}`,
        error.stack,
      );
      throw new InternalServerErrorException('Error updating company');
    }
