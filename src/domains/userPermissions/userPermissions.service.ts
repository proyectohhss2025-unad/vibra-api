...
export class UserPermissionsService {
    ...
    async findByUser(userId: string): Promise<UserPermission[]> {
        return this.userPermissionModel.find({ user: new Types.ObjectId(userId) }).exec();
    }

    async findByPermission(permissionId: string): Promise<UserPermission[]> {
        return this.userPermissionModel.find({ permission: new Types.ObjectId(permissionId) }).exec();
    }

    async create(userPermission: Partial<UserPermission>): Promise<UserPermission> {
        const newUserPermission = new this.userPermissionModel(userPermission);
        return newUserPermission.save();
    }

    async update(id: string, userPermission: Partial<UserPermission>): Promise<UserPermission> {
        return this.userPermissionModel.findByIdAndUpdate(id, userPermission, { new: true }).exec();
    }
