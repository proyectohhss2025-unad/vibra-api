...
export class MongoDBAdminService {
  constructor(
    @InjectModel(MongoDBAdmin.name)
    private readonly mongoDBAdminModel: Model<MongoDBAdmin>,
  ) {}
  ...
  async cloneDatabase(
    ...
    try {
      mongoose.disconnect();
      const originConnection = await mongoose.connect(
        `mongodb://localhost:27017/${originDatabaseName}`,
      );
      ...
      const destinationConnection = await mongoose.connect(
        `mongodb://localhost:27017/${destinationDatabaseName}`,
      );
