...
export class TranslateService {
    constructor(
        @InjectModel(Translate.name) private translateModel: Model<TranslateDocument>,
    ...
    async findAll(filterDto: FilterTranslateDto): Promise<{
        ...
        const totalTranslates = await this.translateModel.countDocuments(query).exec();

        const paginatedTranslates = await this.translateModel.find(query)
            .skip((page - 1) * limit)
            .limit(limit)
            .exec();
    ...
    async findByLanguage(language: string): Promise<Translate> {
        return this.translateModel.findOne({ language, isActive: true, deleted: false }).exec();
    }

    async update(id: string, updateTranslateDto: UpdateTranslateDto): Promise<Translate> {
        return this.translateModel.findByIdAndUpdate(id, updateTranslateDto, { new: true }).exec();
    }

    async remove(id: string, deletedBy: string): Promise<Translate> {
        return this.translateModel.findByIdAndUpdate(
            id,
            {
                deleted: true,
                deletedAt: new Date(),
    ...
    async translateSingleText(text: string, targetLanguage: string): Promise<string> {
        const translation = await this.translateModel.findOne({ language: targetLanguage }).exec();
        return translation?.translations[text] || text;
    }
     ...
     */
    async translateSeveralTexts(texts: string[], targetLanguage: string): Promise<Record<string, string>> {
        const translation = await this.translateModel.findOne({ language: targetLanguage }).exec();
        ...
        this.logger.log(`Translating ${texts.length} texts to ${targetLanguage}...`);
        
        // Validar que todos los textos originales estén en el resultado
        const resultKeys = Object.keys(result);
        if (resultKeys.length !== texts.length) {
            this.logger.warn(`Discrepancia detectada: ${texts.length} textos de entrada pero ${resultKeys.length} en el resultado`);
