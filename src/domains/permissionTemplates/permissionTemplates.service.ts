...
import {
  PermissionTemplate,
  PermissionTemplateDocument,
} from './schemas/permissionTemplate.schema';
import {
  PaginatedResponse,
  PaginationParams,
...
export class PermissionTemplatesService {
  constructor(
    ...
    private permissionTemplateModel: Model<PermissionTemplateDocument>,
  ) {}
  ...
  ): Promise<PaginatedResponse<PermissionTemplate> | PermissionTemplate[]> {
  ...
  async create(
    permissionTemplate: Partial<PermissionTemplate>,
  ): Promise<PermissionTemplate> {
    const createdPermissionTemplate = new this.permissionTemplateModel(
      permissionTemplate,
    );
  ...
  async update(
    ...
    return this.permissionTemplateModel
      .findByIdAndUpdate(id, permissionTemplate, { new: true })
      .exec();
  }
