...
export class NotificationChannelService {
    ...
    async findByTitle(title: string) {
        const channel = await this.notificationChannelModel.findOne({ title }).exec();
        if (!channel) {
            throw new Error('Canal de notificación no encontrado');
        }
        return channel;
    ...
    async remove(id: string) {
        const channel = await this.notificationChannelModel.findByIdAndDelete(id).exec();
        if (!channel) {
            throw new Error('Canal de notificación no encontrado');
        }
        return channel;
    ...
    async createMany(createNotificationChannelDtos: CreateNotificationChannelDto[]) {
