...
export function extractNumberFromString(inputString: string): { number: any, string: any } {
...
export function extractDataFromString(inputString: string): { partA: any, partB: any } {
    const parts = inputString && inputString.length > 0 ? inputString.split(',') : [];
    try {
        if (parts?.length > 1) {
            return { partA: parts[0].trim(), partB: parts[1].trim().replace('Í', 'I') }
        }
        if (parts?.length === 1) {
