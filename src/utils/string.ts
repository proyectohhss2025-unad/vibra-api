...
export function findItem(collection: Permission[] = [], itemId?: string): Permission | undefined {
 ...
 */
export function formatString(chain: string, defaultChain: string = '' + new Date(Date.now())): string {
...
export function normalizeStringArray(array: string[], replaceMap: Record<string, string>): string[] {
...
export function mapArrayToString(obj: any): string {
    ...
    const mappedString = obj.myArray.map((item: any, index: any) => {
...
export function translateNameStatus(statusName: any): string {
    const translate = statusName === 'Sent' ? 'Radicada' :
        statusName === 'Generated' ? 'Generada' :
            statusName === 'Glossed' ? 'Glosada' :
                statusName === 'Approved' ? 'Aprobada' :
                    statusName === 'Paid' ? 'Pago completo' :
                        statusName === 'Partial_payment' ? 'Pago parcial' :
                            statusName === 'Signed' ? 'Firmada' :
                                statusName === 'Verified' ? 'Verificada' : statusName;
    return translate;
}