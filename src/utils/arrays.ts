...
export function getPropertyNameFromJSONObjects<T extends Record<string, any>>(
  objects: T[],
  ...
  return objects.map((obj) => {
    ...
    if (nameKey) {
      ...
      for (const [key, value] of Object.entries(replaceMap)) {
        normalizedNameKey = normalizedNameKey.replace(
          new RegExp(key, 'g'),
          value,
        );
      }
...
export function normalizePropertyNames<T extends Record<string, any>>(
  objects: T[],
  replaceMap: Record<string, string>,
  replaceMapValue: Record<string, string>,
): T[] {
  return objects.map((obj) => {
    ...
    for (const key in obj) {
      if (Object.hasOwnProperty.call(obj, key)) {
        ...
        normalizedObj[normalizedKey] = obj[key]
          .trim()
          .replace(
            new RegExp(Object.keys(replaceMapValue).join('|'), 'g'),
            (match: any) => replaceMapValue[match],
          )
          .trim();
      }
    }
...
export function normalizeArrayStringPropertyNames<
  T extends Record<string, any>,
>(objects: T[], replaceMap: Record<string, string>): T[] {
  return objects.map((obj) => {
    const normalizedObj: { [key: string]: any } = {};
    for (const key in obj) {
      if (Object.hasOwnProperty.call(obj, key)) {
...
export function normalizeArrayString(arr: string[]): string[] {
  return arr.map((element) => {
    ...
    for (const [key, value] of Object.entries(replaceMap)) {
      normalizedElement = normalizedElement.replace(
        new RegExp(key, 'g'),
        value,
      );
    }
...
export function concatInvoiceStatusProperties(statuses: any[]): string {
  try {
    return statuses.reduce((concat, item) => {
      return (
        concat +
        `${translateNameStatus(item?.oldInvoiceStatus?.name)} a ${translateNameStatus(item?.currentInvoiceStatus?.name)} por ${item.createdBy} el ${formatDate(item?.createdAt ?? null, 'YYYY-MM-DD')} `
      );
    }, '');
  } catch (error) {
    return '';
...
export function getPropertyNameFromJSONObjects_<T extends Record<string, any>>(
  objects: T[],
  replaceMap: Record<string, string>,
): string[] {
  return objects.map((obj) => {
...
export function getPropertyNames<T extends Record<string, any>>(
  objects: T[],
): string[] {
  if (objects.length === 0) {
    return [];
...
export function getCompareArray(
  arr1: string[],
  arr2: string[],
): { coincide: boolean; doNotMatch: string[] } {
  const coincide =
...
export function getCompareArrays(
  arr1: any[],
  arr2: any[],
): { coincide: boolean; doNotMatchA: any; doNotMatchB: any } {
  const coincide =
    arr1.every((element) => arr2.includes(element)) &&
    arr2.every((element) => arr1.includes(element));
