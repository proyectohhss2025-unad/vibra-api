  ...
  try {
    ...
    if (!enDev) {
      ...
      sgMail
        ...
        .catch((error: any) => {
          logger.error(
            'Error en el envio del correo de notificación de OTP',
            error,
            email,
          );
        });
    ...
    if (enDev) {
      logger.info(
        'No es posible enviar el correo desde un ambiente de desarrollo',
      );
    }
  } catch (error) {
...
export const sendPassGenerateAndSendEmail = async (
  email: string,
  ...
  try {
    if (!enDev) {
      ...
      sgMail
        ...
        .catch((error: any) => {
          logger.error(
            'Error en el envio del correo de notificación de nueva contraseña',
            error,
            email,
          );
        });
...
export const sendInfoJobs = async (
  email: string,
  message: string,
  enDev = true,
) => {
  try {
    if (!enDev) {
      ...
      sgMail
        ...
        .catch((error: any) => {
          logger.error(
            'Error en el envio del correo con la notificación del jobs generado',
            { error, email },
          );
        });
    }
...
export const sendInfoAccountsReceivable = async (
  email: string,
  message: string,
  accountsReceivable: any,
  amountTotalPayments: number,
  enDev = false,
) => {
  try {
    if (!enDev) {
      ...
      const msg = {
        ...
        <div id='reportAccountReceivable' class="container">
            ...
            <table>
                ...
                <tbody>` +
          accountsReceivable?.participant?.invoiceItems?.map(
            (item) =>
              `<tr>
          ...
          `</tbody>
            ...
            <div className="total">
                <p>Subtotal: ${formatToLocalCurrency(accountsReceivable?.participant?.totalValue, 'en-US')}</p>
                <p>IVA (19%): ${formatToLocalCurrency(accountsReceivable?.participant?.totalValue * 0.19, 'en-US')}</p>
                <p>Total: ${formatToLocalCurrency(accountsReceivable?.participant?.totalValue, 'en-US')}</p>
            </div>
            <h2>Pagos</h2>
            <table>
                <thead>
                ...
                <tbody>` +
          accountsReceivable?.paymentsReceived?.map(
            (payment) =>
              `<tr>
      ...
      sgMail
        ...
        .catch((error: any) => {
          logger.error(
            'Error en el envio del correo con la notificación de cuenta de cobro',
            { error, email },
          );
        });
    }
