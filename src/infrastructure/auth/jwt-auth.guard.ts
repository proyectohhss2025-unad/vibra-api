...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  canActivate(context: ExecutionContext) {
    ...
    const xAccessToken: string | undefined = (request.headers as any)[
      'x-access-token'
    ];
    ...
    this.appLogger.debug(
      `JwtAuthGuard IN -> ${method} ${url} | authorization=${redact(authHeader)} | x-access-token=${redact(xAccessToken)}`,
    );

    if (
      (!authHeader || typeof authHeader !== 'string') &&
      (!xAccessToken || typeof xAccessToken !== 'string')
    ) {
    ...
    if (!sanitizedToken) {
      this.appLogger.warn(
        `Invalid Authorization header format for ${method} ${url}`,
      );
    ...
    this.appLogger.debug(
      `JwtAuthGuard normalized header -> ${method} ${url} | authorization=Bearer ${sanitizedToken.split('.')[0]}.***`,
    );
  ...
  handleRequest(err: any, user: any, info: any) {
    if (err || !user) {
      const msg =
        info?.message || (err?.message as string) || 'Invalid or expired token';
