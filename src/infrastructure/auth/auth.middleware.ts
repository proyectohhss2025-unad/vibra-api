...
export class AuthMiddleware implements NestMiddleware {
    ...
    use(req: Request, res: Response, next: () => void) {
        ...
        const rawAuthHeader = req.headers['authorization'] || (req.headers as any)['Authorization'];
        ...
        this.logger.debug(`AuthMiddleware IN -> ${method} ${url} | authorization=${redact(rawAuthHeader as string)} | x-access-token=${redact(rawXAccessToken as string)}`);

        // NO validar, NO reescribir headers, NO devolver 401. La autorización la maneja JwtAuthGuard.
        this.logger.debug(`AuthMiddleware PASS -> ${method} ${url} | forwarded to guards (no auth logic here)`);
        next();
    }
}